 ...
 * @returns {number}
 */
const calculateSubGridLevel = (fuelGrid, x, y, gridSize) => {
  let sum = 0;
...
const getMaxPowerGridPosition = fuelGrid => {
  ...
  for (let size = 1; size <= fuelGrid.length; ++size) {
    for (let x = 0; x < fuelGrid.length - (size - 1); ++x) {
      for (let y = 0; y < fuelGrid[x].length - (size - 1); ++y) {
        let powerLevel = calculateSubGridLevel(fuelGrid, x, y, size);

        if (powerLevel > maxPowerLevel) {
          maxPowerLevel = powerLevel;
          maxPowerPosition = { x, y, size };
        }
      }
    }
  }
...
/**
 * Calculation solution of puzzle 2
 */
const calculateSolution2 = () => {
  ...
  console.log(
    `Solution 2: ${maxGridPos.x + 1}, ${maxGridPos.y + 1}, ${maxGridPos.size}`
  );
};
